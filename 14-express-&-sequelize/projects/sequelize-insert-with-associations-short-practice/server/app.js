...
app.post('/bands/:bandId/musicians', async (req, res, next) => {
    ...
    try {
        ...
        // Find the Band indicated by the route param
        const band = await Band.findByPk(bandId);

        // Create a new Musician using the Band association's creator method
...
app.post('/musicians/:musicianId/instruments', async (req, res, next) => {
    ...
    try {
        ...
        // Find the Musician indicated by the route param
        const musician = await Musician.findByPk(musicianId);

        // Add the Instruments to the Musician using the Musician association's add method
        await musician.addInstruments(InstrumentIds);


        res.status(201).json({
            message: 'Associated ${musician.name} with instruments ${instrumentIds}.',
        });
    } catch (error) {
        next(error)
    }
